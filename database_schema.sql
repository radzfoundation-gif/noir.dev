-- ============================================================================
-- NOIR AI - Complete Supabase Database Schema
-- Version: 2.0.2
-- Created: 2025-02-05
-- Description: Full database schema with safe DROP/CREATE patterns
-- Usage: Copy & paste entire script to Supabase SQL Editor
-- ============================================================================
...
DROP POLICY IF EXISTS "Users can delete versions of their projects" ON project_versions;
...
DROP POLICY IF EXISTS "Allow anonymous to join waitlist" ON waitlist;
CREATE POLICY "Allow anonymous to join waitlist"
    ON waitlist FOR INSERT
    WITH CHECK (true);

DROP POLICY IF EXISTS "Allow authenticated to view waitlist" ON waitlist;
CREATE POLICY "Allow authenticated to view waitlist"
    ON waitlist FOR SELECT
    USING (true);

DROP POLICY IF EXISTS "Allow delete on waitlist" ON waitlist;
...
-- - 21 tables created
-- - 30+ indexes created
-- - 50+ RLS policies created (with DROP IF EXISTS pattern)
-- - 8 triggers created (with DROP IF EXISTS pattern)
-- - 1 function created
-- - 2 seed records inserted
-- - 11 tables added to realtime publication
